...
#        print(run_time)
        return  s
#        self.LOG('{} dataset with {} method get  {:.4f} max-fmeasure,  {:.4f} S-measure..\n'.format(self.dataset, self.method, max_f, s))
#        return '[cost:{:.4f}s]{} dataset with {} method get , {:.4f} max-Emeasure, {:.4f} S-measure..'.format(time.time()-start_time, self.dataset, self.method, max_f, s),max_f
    def Eval_mae(self):
...
#                print('object',self._S_object(pred, gt))
#                print('region',self._S_region(pred, gt))
                if Q.item() < 0:
                    Q = torch.Tensor([0.0])
#                    Q = [0.0]
