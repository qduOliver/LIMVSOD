...
for a in range(0, len(dataset_list)):
    ...
    path_total = r'F:\source\tableV\6Smeasure_result\11/%s/%s/' % (pt,dataset)

    videos = os.listdir(path_total)
    for i in range(0, len(videos)):
        video = videos[i]

        select_save_path_end_bi = r'F:\source\tableV\6Smeasure_result\KFS_novalue/%s/%s/%s/' % (pt,dataset, video)
        select_save_path_value = r'F:\source\tableV\6Smeasure_result\KFS_value/%s/%s/%s/' % (pt,dataset, video)
