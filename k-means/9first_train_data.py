...
for sj in range(0, len(dataset_list)):
    ...
    total_apth = r'F:\source\Visal3\8julei0_1_ed/%s/' % (dataset)
    ...
    for i in range(0, len(video_list)):
        ...
        for j in range(0, len(clas_list)):
            ...
            for k in range(0, len(imgs)):
                ...
                msv = float(img_name.split('sec')[1][:-4])
            ...
            save_path_train = r'F:\source\Visal3\9julei0_1_ed_train/%s/%s/%s/' % (
            dataset, vidoe_name, cla_name)
            save_path_test = r'F:\source\Visal3\9julei0_1_ed_test/%s/%s/%s/' % (dataset, vidoe_name, cla_name)
