...
def display(preds, imgs,folder,name, imshow=True, imwrite=False):
    txt_path = './txt/new_1_flo/%s/M7_0.01_0.2_top10/'%(dataset)+folder
    ...
    for i in range(len(imgs)):
            ...
            if imwrite:
                save_path= './test/new_1_flo/M7_0.01_0.2_top3/%s/%s'%(dataset,folder)
...
for ss in range(0,len(dataset_list)):
    ...
    total_path=r'F:\dataset\flow\pwc_net_jpg/%s/'%(dataset)
    folers = os.listdir(total_path)
    for k in range(31,len(folers)):
        folder = folers[k]
        
        # img_path =r'%s/%s/Imgs/'%(total_path,folder)
        img_path =r'%s/%s/'%(total_path,folder)
