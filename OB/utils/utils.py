...
def preprocess(*image_path,img_name, max_size=512, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    ...
    return ori_imgs, framed_imgs, framed_metas,img_name
...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    ...
    # transformed_anchors  [1,441936,4]
    transformed_anchors = regressBoxes(anchors, regression)
    # transformed_anchors [1,441936,4]
    transformed_anchors = clipBoxes(transformed_anchors, x)
    #scores  [1,441936,1]
    scores = torch.max(classification, dim=2, keepdim=True)[0]
    # scores_over_thresh [1,441936]
    ...
    for i in range(x.shape[0]):
        if scores_over_thresh[i].sum() == 0:
            ...
            continue
        # classification_per [90,677]
        classification_per = classification[i, scores_over_thresh[i, :], ...].permute(1, 0)
        # transformed_anchors_per  [677,4]
...
def plot_one_box(img, coord, label=None, score=None, color=None, line_thickness=None):
    ...
    if label:
        ...
        # cv2.putText(img, '{:.0%}'.format(score), (c1[0],c1[1] - 2), 0, float(tl) / 3, [0, 0, 0], thickness=tf, lineType=cv2.FONT_HERSHEY_SIMPLEX)
