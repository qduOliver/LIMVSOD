...
for ss in range(0, len(dataset_list)):
    ...
    path_total = r'./txt/total/%s/' % (dataset)
    ...
    for a in range(0, len(folders)):
        ...
        for b in range(0, len(imgs)):
            ...
            save_path = r'.\nms/img/%s/%s' % (dataset, folder)
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            save_path_txt = r'.\nms/txt/%s/%s' % (dataset, folder)

            if not os.path.exists(save_path_txt):
                os.makedirs(save_path_txt)
            f = open(save_path_txt + '/' + img_name, 'a')
            img_path = r'D:\source\VSOD_dataset\%s/%s\Imgs/%s' % (dataset, folder, img_name[:-4])
            # print(img_name[:-4])
