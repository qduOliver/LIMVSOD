...
def train_model(model, criterion, optimizer, scheduler, video, num_epochs, csv_name, dataset):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            if not os.path.exists('julei8_m8/train_julei8_forth/%s_nofix_noaug_adp/%s/' % (csv_name, dataset) + video):
                os.makedirs('julei8_m8/train_julei8_forth/%s_nofix_noaug_adp/%s/' % (csv_name, dataset) + video)

            # if (epoch+1) % 5 == 0:
            if (epoch) % 2 == 0:
                model.load_state_dict(copy.deepcopy(model.state_dict()))
                torch.save(model.state_dict(),
                'julei8_m8/train_julei8_forth/{}_nofix_noaug_adp/{}/{}/{}.pt'.format(csv_name, dataset, video,
                                                                                             epoch))
    #
    ...
    torch.save(model.state_dict(),
               'julei8_m8/train_julei8_forth/{}_nofix_noaug_adp/{}/{}/{}.pt'.format(csv_name, dataset, video, epoch))
    print('Best val classes Acc: {:.2%}'.format(best_acc))
...
for aa in range(0, len(dataset_list)):
    ...
    csv_name = 'csv_name_julei8'
    # train_csv_path = './csv_patch_train_adp/%s/%s/'%(dataset,csv_name)
    train_csv_path = 'F:\source\Visal17/10classifier/train_csv_julei8_forth/%s/' % (dataset)
    cvss = os.listdir(train_csv_path)
    for h in range(0, len(cvss)):
        video = cvss[h]
        print('video', video)
        TRAIN_ANNO = train_csv_path + '/%s' % (video)
        # VAL_ANNO = './test_csv_julei6/%s/%s'%(dataset,video)
        ...
        model, Loss_list, Accuracy_list_classes = train_model(network, criterion, optimizer, exp_lr_scheduler,
                                                              save_video, num_epochs=20, csv_name=csv_name,
                                                              dataset=dataset)
