...
def test_model(model, weight_path, dataset, test_data, video):
    ...
    for phase in ['test']:
        ...
        for idx, data in enumerate(data_loaders[phase]):
            ...
            cla = data['cla_name'][0]
            save_path = r'F:\source\Visal17\14first_test_result/%s/%s' % (dataset, video[:-4])
            # save_path = r'G:\source\MyResult11\julei_8S\FClassier_fenAllpatch/%s/%s_result/%s' % (dataset, test_data, video[:-4])
            ...
            aa = save_path + '/' + img_name[0][:-4] + '_T1_' + str(preds_classes) + '.jpg'
            #            print(aa)
            #            print('aa',aa)
...
for aa in range(0, len(dataset_list)):
    ...
    test_data = 'test_%s' % dataset
    total_path = 'F:\source\Visal17/13test_csv_julei8_first/%s/' % (dataset)
    ...
    for i in range(0, len(videos)):
        ...
        weight_path = 'csv_name_julei8_nofix_noaug_adp'
        # shi 训练好的网络 吱吱吱吱吱吱吱吱吱吱吱吱吱吱吱吱吱吱吱
        pretrained_dict = torch.load(
        r'G:\spyder_workpeace_song\total\class_classification\julei8_m8\train_julei8_first/%s/%s/%s/14.pt' % (weight_path, dataset, video[:-4]))
